 ...
 *   - "인증로그"  : 없으면 스크립트가 첫 인증번호 발급 시 자동 생성합니다. (사번/인증번호/만료시각)
 *
 * 설치 방법은 README.md 참고.
 */
...
const SHEET_OTP = '인증로그'; // 없으면 자동 생성. 인증번호를 CacheService 대신 시트에 저장(더 안정적)
...
/** ---- 인증번호 저장소 (Google Sheet 기반, CacheService보다 안정적) ---- */
function ensureOtpSheet_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName(SHEET_OTP);
  if (!sheet) {
    sheet = ss.insertSheet(SHEET_OTP);
    sheet.appendRow(['사번', '인증번호', '만료시각']);
    sheet.setFrozenRows(1);
  }
  return sheet;
}

function findOtpRow_(sheet, sabun) {
  const rows = sheet.getDataRange().getValues();
  for (let i = 1; i < rows.length; i++) {
    if (String(rows[i][0]).trim() === String(sabun).trim()) return i + 1; // 1-based row index
  }
  return -1;
}

function setOtp_(sabun, code, ttlSeconds) {
  const sheet = ensureOtpSheet_();
  const expiresAt = new Date(Date.now() + ttlSeconds * 1000);
  const rowIdx = findOtpRow_(sheet, sabun);
  if (rowIdx > 0) {
    sheet.getRange(rowIdx, 1, 1, 3).setValues([[String(sabun).trim(), code, expiresAt]]);
  } else {
    sheet.appendRow([String(sabun).trim(), code, expiresAt]);
  }
}

/** 유효한(만료 안 된) 인증번호가 있으면 반환, 없으면 null */
function getValidOtp_(sabun) {
  const sheet = ensureOtpSheet_();
  const rowIdx = findOtpRow_(sheet, sabun);
  if (rowIdx < 0) return null;
  const row = sheet.getRange(rowIdx, 1, 1, 3).getValues()[0];
  const code = String(row[1]);
  const expiresAt = new Date(row[2]);
  if (isNaN(expiresAt.getTime()) || new Date() > expiresAt) return null;
  return code;
}

function clearOtp_(sabun) {
  const sheet = ensureOtpSheet_();
  const rowIdx = findOtpRow_(sheet, sabun);
  if (rowIdx > 0) sheet.deleteRow(rowIdx);
}

/** 사번 → 인증번호 발급 및 메일 발송 (10분 유효) */
...
function verifyOtp_(sabun, code) {
  ...
  sabun = String(sabun).trim();
  const stored = getValidOtp_(sabun);
